...
namespace Broker.Function
{
    public class Startup : FunctionsStartup
    {
        ...
        public const string SERVICE_NAME = "broker-function-listener-greenkoncept";
        public override void Configure(IFunctionsHostBuilder builder)
        {
            // builder.Services.AddApplicationInsightsTelemetry();
            builder.Services.AddRabbitMQ(SERVICE_NAME);
            builder.Services.AddHttpClient(HttpClientName.GREEN_KONCEPT, (service, client) =>
            {
                var configuration = service.GetRequiredService<IConfiguration>();
                client.BaseAddress = new Uri(configuration["GreenKoncept:Endpoint"]);
            });
            ...
            builder.Services.AddSingleton<IGreenKonceptService, GreenKonceptService>();
