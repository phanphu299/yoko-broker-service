...
var pglConnectionString = EnvironmentVariable("PglConnectionStrings__Default");
var redisConnectionString = EnvironmentVariable("RedisConnectionString");
...
// Migrate TOPIC
///////////////////////////////////////////////////////////////////////////////
Task("Migrate-Topic-Database")
    .Description("Migrate topic database")
    .Does(() =>
{
    if(base64ConnectionString){
        var base64EncodedBytes = System.Convert.FromBase64String(connectionString);
        ...
        var pglBase64EncodedBytes = System.Convert.FromBase64String(pglConnectionString);
        pglConnectionString =  System.Text.Encoding.UTF8.GetString(pglBase64EncodedBytes);
    }

    connectionString = connectionString?.Replace(" ", "%20");
    pglConnectionString = pglConnectionString?.Replace(" ", "%20");

    var filePrj = GetFiles("./**/*TopicMigration.csproj");

    var settings = new DotNetCoreRunSettings()
    {
        ArgumentCustomization = args => args.Append("database")
                                            .Append(connectionString?.Replace("{{projectId}}", projectId.Replace("-","").ToLower()).Replace("{{subscriptionId}}", subscriptionId.Replace("-","").ToLower()).Replace("{{tenantId}}", tenantId.Replace("-","").ToLower()))
                                            .Append(pglConnectionString?.Replace("{{projectId}}", projectId.Replace("-","").ToLower()).Replace("{{subscriptionId}}", subscriptionId.Replace("-","").ToLower()).Replace("{{tenantId}}", tenantId.Replace("-","").ToLower())),

    };

    DotNetCoreRun(filePrj.First().ToString(), settings);
});


Task("Migrate-Topic-Redis")
    .Description("Migrate-Topic-Redis")
    .Does(() =>
{
    if (base64ConnectionString)
    {
        var base64EncodedBytes = System.Convert.FromBase64String(connectionString);
        connectionString = System.Text.Encoding.UTF8.GetString(base64EncodedBytes);
        var redisBase64EncodedBytes = System.Convert.FromBase64String(redisConnectionString);
        redisConnectionString = System.Text.Encoding.UTF8.GetString(redisBase64EncodedBytes);
    }

    connectionString = connectionString?.Replace(" ", "%20");
    redisConnectionString = redisConnectionString?.Replace(" ", "%20");

    var settings = new DotNetCoreRunSettings()
    {
        ArgumentCustomization = args => args.Append("redis")
                                            .Append(connectionString?.Replace("{{projectId}}", projectId.Replace("-", "").ToLower()).Replace("{{subscriptionId}}", subscriptionId.Replace("-", "").ToLower()).Replace("{{tenantId}}", tenantId.Replace("-", "").ToLower()))
                                            .Append(redisConnectionString),

    };

    var filePrj = GetFiles("./**/*TopicMigration.csproj");
    Console.WriteLine(filePrj.First().ToString());
    DotNetCoreRun(filePrj.First().ToString(), settings);
});
