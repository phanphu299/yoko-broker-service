...
namespace Function.Filter
{
    public class AzureClientCrendetialAuthentication : DelegatingHandler
    {
        ...
        private async Task<TokenResponse> AccquireTokenAsync(bool clearCache = false)
        {
            ...
            {
                throw new System.Exception($"Azure__ClientId is not configured in appsettings.");
            }
            if (string.IsNullOrEmpty(clientSecret))
            {
                throw new System.Exception($"Azure__ClientSecret is not configured in appsettings.");
            }
            if (string.IsNullOrEmpty(tenantId))
            {
                throw new System.Exception($"Azure__TenantId is not configured in appsettings.");
            }
            ...
            var tokenResponse = await tokenRequest.Content.ReadAsByteArrayAsync();
            response = tokenResponse.Deserialize<TokenResponse>();
